...
- `settings` — key-value; the `power` key holds the last selected calculation settings (`RideSettings`: `cda` position + `crr` surface/tires/pressure) — the next loaded ride is initialized with them.
